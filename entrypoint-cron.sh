...
if [ -n "$TOKEN_ANALYTICS_B64" ]; then
    echo "$TOKEN_ANALYTICS_B64" | base64 -d > /app/token_analytics.pickle
    echo "OK: token_analytics.pickle"
fi
...
printenv | grep -E '^(DEEPSEEK_API_KEY|ANTHROPIC_API_KEY|TELEGRAM_BOT_TOKEN|TELEGRAM_CHAT_ID|PYTHONPATH|PATH|HOME)=' | sed 's/^/export /' > /app/.env.cron
